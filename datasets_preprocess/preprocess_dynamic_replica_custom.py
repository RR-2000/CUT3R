...
def process_split_data(args):
    ...
    for seq_name in tqdm(seq_annot.keys(), desc=f"Processing split '{split}'"):
        ...
        for cam in ["left"]:
            ...
            out_cam_dir = osp.join(out_dir, f"{seq_name}_source_{cam}", 'camera')
            ...
            for idx, framedata in tqdm(
                enumerate(seq_annot[seq_name][cam]), desc=f"Seq {seq_name} [{cam}]", leave=False
            ):
                ...
                out_cam_path = osp.join(out_cam_dir, f"{idx:04d}.npz")
