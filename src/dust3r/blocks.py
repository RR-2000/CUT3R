...
class Attention(nn.Module):
    ...
    def __init__(
        ...
        self.prev_q_cam = None
        self.prev_k_cam = None

    def reset_prev_cam_tokens(self):
        self.prev_q_cam = None
        self.prev_k_cam = None

    def forward(self, x, xpos, mask=None, q_masker=None, k_masker=None):
        B, N, C = x.shape
...
class CrossAttention(nn.Module):
    ...
    def forward(self, query, key, value, qpos, kpos, mask=None, q_masker=None, k_masker=None):
        B, Nq, C = query.shape
        Nk = key.shape[1]
        Nv = value.shape[1]
...
class CustomDecoderBlock(nn.Module):
    ...
    def forward(self, x, y, z, xpos, ypos, mask=None):
        x = x + self.drop_path(self.attn(self.norm1(x), xpos, mask=mask))
        y_ = self.norm_y(y)
        z_ = self.norm_z(z)
        x = x + self.drop_path(self.cross_attn(self.norm2(x), y_, z_, xpos, ypos, mask=mask))
        x = x + self.drop_path(self.mlp(self.norm3(x)))
        return x, y
